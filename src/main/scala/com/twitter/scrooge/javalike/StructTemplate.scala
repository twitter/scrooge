...
trait StructTemplate extends Generator {
  ...
  lazy val readBasicTemplate = templates("readBasic").generate {
    binding: Binding[BaseType] =>
  ...
  def readListTemplate = templates("readList").generate {
    binding: Binding[ListType] =>
      ...
      Dictionary(
        ...
        "eltReader" -> v(readTemplate(Binding("element", t.eltType)).indent(2))
      )
  }

  def readSetTemplate = templates("readSet").generate {
    binding: Binding[SetType] =>
      val Binding(name, t) = binding
      Dictionary(
        "name" -> v(name),
        "eltType" -> v(typeName(t.eltType)),
  ...
  lazy val readMapTemplate = templates("readMap").generate {
    binding: Binding[MapType] =>
      val Binding(name, t) = binding
      Dictionary(
        "name" -> v(name),
        "keyType" -> v(typeName(t.keyType)),
        "valueType" -> v(typeName(t.valueType)),
        "keyReader" -> v(readTemplate(Binding("key", t.keyType)).indent(2)),
        "valueReader" -> v(readTemplate(Binding("value", t.valueType)).indent(2))
      )
  }

  lazy val readStructTemplate = templates("readStruct").generate {
    binding: Binding[StructType] =>
      val Binding(name, t) = binding
      Dictionary(
        "name" -> v(name),
        "type" -> v(t.name)
  ...
  lazy val readEnumTemplate = templates("readEnum").generate {
    binding: Binding[EnumType] =>
      val Binding(name, t) = binding
      Dictionary(
        "name" -> v(name),
        "type" -> v(t.name)
  ...
  lazy val readFieldTemplate = templates("readField").generate {
    f: Field =>
      Dictionary(
        "id" -> v(f.id.toString),
        "name" -> v(f.name),
        ...
        "valueReader" -> v(readTemplate(Binding(f.name, f.`type`)).indent(4)),
  ...
  lazy val writeBasicTemplate = templates("writeBasic").generate {
    binding: Binding[BaseType] =>
      val Binding(name, t) = binding
      Dictionary(
  ...
  lazy val writeListTemplate = templates("writeList").generate {
    binding: Binding[ListType] =>
      val Binding(name, t) = binding
      Dictionary(
        "eltType" -> v(constType(t.eltType)),
        "eltWriter" -> v(writeTemplate(Binding("element", t.eltType)).indent(1))
      )
  }

  lazy val writeSetTemplate = templates("writeSet").generate {
    binding: Binding[SetType] =>
      val Binding(name, t) = binding
      ...
      Dictionary(
        "eltName" -> eltName,
        "eltType" -> v(constType(t.eltType)),
        "eltWriter" -> v(writeTemplate(Binding(eltName, t.eltType)).indent(1))
      )
  }

  lazy val writeMapTemplate = templates("writeMap").generate {
    binding: Binding[MapType] =>
      val Binding(name, t) = binding
      val keyName = "_key"
      ...
      Dictionary(
        ...
        "keyName" -> keyName,
        "valueName" -> valueName,
        "keyWriter" -> v(writeTemplate(Binding(keyName, t.keyType)).indent(2)),
        "valueWriter" -> v(writeTemplate(Binding(valueName, t.valueType)).indent(2))
      )
  }

  lazy val writeStructTemplate = templates("writeStruct").generate {
    binding: Binding[StructType] =>
      val Binding(name, t) = binding
      Dictionary()
  }

  lazy val writeEnumTemplate = templates("writeEnum").generate {
    binding: Binding[EnumType] =>
      val Binding(name, t) = binding
      Dictionary()
  }
  ...
  lazy val writeFieldTemplate = templates("writeField").generate {
    f: Field =>
      ...
      Dictionary(
        ...
        "valueWriter" -> v(writeTemplate(Binding(f.name, f.`type`)).indent(1))
      )
  }

  // ----- struct

  lazy val structTemplate = templates("struct").generate {
    struct: StructLike =>
