...
class ThriftParser(
  ...
  implicit val logger: Logger = Logger.getLogger(classOf[ThriftParser].getName))
  ...
  lazy val mapType: Parser[MapType] =
    ("map" ~> opt(cppType) <~ "<") ~
      ...
      (fieldType ~ defaultedAnnotations <~ ">") ^^ {
      case cpp ~ (key ~ keyAnnotations) ~ (value ~ valueAnnotations) =>
        MapType(
          AnnotatedFieldType.wrap(key, keyAnnotations),
          AnnotatedFieldType.wrap(value, valueAnnotations),
          cpp)
    }

  lazy val setType: Parser[SetType] =
    ("set" ~> opt(cppType)) ~ ("<" ~> fieldType ~ defaultedAnnotations <~ ">") ^^ {
      case cpp ~ (t ~ annotations) => SetType(AnnotatedFieldType.wrap(t, annotations), cpp)
    }

  lazy val listType: Parser[ListType] =
    ("list" ~ "<") ~> (fieldType ~ defaultedAnnotations <~ ">") ~ opt(cppType) ^^ {
      case (t ~ annotations) ~ cpp => ListType(AnnotatedFieldType.wrap(t, annotations), cpp)
    }
