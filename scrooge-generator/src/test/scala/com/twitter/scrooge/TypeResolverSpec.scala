...
class TypeResolverSpec extends SpecificationWithJUnit {
  "TypeResolve" should {
    ...
    val struct = Struct(SimpleID("BlahBlah"), Seq(
      Field(1, SimpleID("baby"), TI16),
      Field(2, SimpleID("mama"), TI32),
      Field(3, SimpleID("papa"), TI64),
      Field(4, SimpleID("pupu"), enumRef)
    ), None)
    val structType = new StructType(struct)
    val structRef = ReferenceType(struct.sid)
    val ex = Exception_(SimpleID("Boom"), Seq(Field(1, SimpleID("msg"), enumRef)), None)
    ...
    "transform a Field" in {
      val field = Field(42, SimpleID("foo"), structRef)
    ...
    "transform a Function" in {
      val field = Field(1, SimpleID("foo"), structRef)
      val ex = Field(2, SimpleID("ex"), structRef)
      val fun = Function(SimpleID("foo"),  structRef, Seq(field), Seq(ex), None)
      resolver(fun) mustEqual
        Function(SimpleID("foo"), resolver(fun.funcType), Seq(resolver(field)), Seq(resolver(ex)), None)
    }
    ...
    "transform a Service" in {
      val fun = Function(SimpleID("foo"), structRef, Seq(Field(1, SimpleID("foo"), structRef)), Nil, None)
    ...
    "resolve a typedef from an included scope" in {
      val oneInt = Struct(SimpleID("OneInt"), Seq(Field(1, SimpleID("id"), TI32, None, Requiredness.Default)), None)
      ...
      val collectionStruct = Struct(SimpleID("IntCollection"), Seq(
        Field(1, SimpleID("scores1"), ReferenceType(Identifier("typedef1.ManyInts")), None, Requiredness.Default),
        Field(2, SimpleID("scores2"), SetType(ReferenceType(Identifier("typedef1.OneInt")), None), None, Requiredness.Default)
      ), None)
      ...
      resolvedDoc.defs(0) must beLike {
        case Struct(_, fields, _) => {
          fields(0) must beLike {
            case Field(1, _, ListType(StructType(_, Some(SimpleID("typedef1"))), None), _, _) => true
          }
          fields(1) must beLike {
            case Field(2, _, SetType(StructType(_, Some(SimpleID("typedef1"))), None), _, _) => true
          }
        }
      }
    }
